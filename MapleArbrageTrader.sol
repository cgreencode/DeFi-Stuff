...
contract MapleTrader{
    ...
    // verify the interaction is with the creator of the contract
    modifier onlyOwner() {
        if (msg.sender != owner) {
            throw;
        }
    ...
    // main arb logic
     function kyberToUniSwapArb(address fromAddress, address uniSwapContract, uint theAmount) public payable onlyOwner returns (bool){
        ...
        // instantiate the Uniswap interface
        ...
        // convert stablecoin to ETH (BUY action)
        uint ethBack = swapTokenToEther1(proxy, address1 , theAmount, msg.sender);

        // convert ETH to stablecoin (SELL action)
    ...
    // only the creator of this contract can interface with this function
    function withdrawETHAndTokens() onlyOwner{

        msg.sender.send(address(this).balance); // transfer remaining ETH balance to owner
         ERC20 daiToken = ERC20(daiAddress);
        uint256 currentTokenBalance = daiToken.balanceOf(this);
        daiToken.transfer(msg.sender, currentTokenBalance); // transfer remaining DAI balance to owner
    }

    // retrieves sell price from Kyber
    ...
    // retrieves buy price from Uniswap
